...
int xdp_bridge_postprocessing(struct xdp_md *ctx)
{
	...
	if (mpls_encapsulation || udp_ip_encapsulation) {
		...
		if (mpls_encapsulation) {
			total_encapsulation_size += sizeof(mpls_stack_entry) + 4 /* DetNet CW */;
		}
		...
		// adjust without control word that was added before redirect
		if (bpf_xdp_adjust_head(ctx, -total_encapsulation_size+4)) {
			if (debug_output) {
				bpf_printk(
