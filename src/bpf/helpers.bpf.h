...
static inline int stream_identification(void *data, void *data_end, u16 vlan_tci_override, struct null_stream_identification *stream_id, struct stream **stream)
{
	...
	if (vlan_tci_override) {
		...
		if (vlan_proto != ETH_P_8021Q && vlan_proto != ETH_P_8021AD) {
			if (debug_output) {
				bpf_printk("No VLAN header in packet. Consider disabling VLAN offload. Assuming VLAN ID 0");
			}

			stream_id->vlan_identifier = 0;
		} else {
			stream_id->vlan_identifier = bpf_ntohs(eth->h_vlan_TCI) & 0xFFF;
		}
	}
	...
	*stream = bpf_map_lookup_elem(&streams, &stream_id);
	if (!*stream) {
		...
		struct stream *stream = bpf_map_lookup_elem(&streams, &default_stream_id);

		if (!stream) {
			if (debug_output) {
				// should never happen, if no explicit stream is available,
				// the default stream should be available
				bpf_printk("Dropping packet due to internal error when loading default stream");
			}
			return -1;
		}
	}
