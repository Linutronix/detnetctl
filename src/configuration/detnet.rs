...
impl FillDefaults for Flow {
    ...
    fn fill_defaults(&mut self) -> Result<()> {
        if let Some(incoming_app_flows) = &mut self.incoming_app_flows {
            for incoming_app_flow in incoming_app_flows {
                incoming_app_flow.fill_defaults()?;
            }
        }

        if let Some(outgoing_forwarding) = &mut self.outgoing_forwarding {
            for ofwd in outgoing_forwarding {
                ofwd.fill_defaults()?;
            }
        }

        if let Some(incoming_forwarding) = &mut self.incoming_forwarding {
            for ifwd in incoming_forwarding {
                ifwd.fill_defaults()?;
            }
        }

        if let Some(outgoing_app_flows) = &mut self.outgoing_app_flows {
            for outgoing_app_flow in outgoing_app_flows {
                outgoing_app_flow.fill_defaults()?;
            }
        }
...
/// Flow identification
/// Currently only UDP source port and MPLS S-Label is supported
#[derive(
    Debug,
    PartialEq,
    Eq,
...
impl FillDefaults for FlowIdentification {
    /// Fill unset fields with defaults.
...
pub struct IncomingForwarding {
    ...
    /// Identification
    identification: Option<FlowIdentification>,
}
...
impl FillDefaults for IncomingForwarding {
    ...
    fn fill_defaults(&mut self) -> Result<()> {
        if self.identification.is_some() {
            fill_struct_defaults!(self, identification, FlowIdentificationBuilder);
        }
